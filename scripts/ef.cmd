...
:: Find projects (Data + first Web SDK service)
for /r "%ROOT_DIR%\src\Services" %%f in (*.csproj) do (
    echo %%~nxf | findstr /i /c:".Data.csproj" >nul && if not defined DATA_PROJECT set "DATA_PROJECT=%%f"
    findstr /c:"Microsoft.NET.Sdk.Web" "%%f" >nul && if not defined API_PROJECT set "API_PROJECT=%%f"
)

if not defined DATA_PROJECT (
    echo [ERROR] Could not find Data project under src\Services
    exit /b 1
)

if not defined API_PROJECT (
    echo [ERROR] Could not find Web API project under src\Services
    exit /b 1
)
