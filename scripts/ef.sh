...
find_projects() {
    ...
    while IFS= read -r -d '' csproj; do
        ...
        if [ -z "$DATA_PROJECT" ] && echo "$base_name" | grep -qi '\.Data\.csproj$'; then
            DATA_PROJECT="$csproj"
        fi
    ...
    done < <(find "$ROOT_DIR/src/Services" -type f -name "*.csproj" -print0)

    if [ -z "$DATA_PROJECT" ]; then
        echo -e "${RED}Error: Could not find Data project under src/Services${NC}"
        exit 1
    fi

    if [ -z "$API_PROJECT" ]; then
        echo -e "${RED}Error: Could not find Web API project under src/Services${NC}"
        exit 1
    fi
